...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        # return closest ghost position based on distance from pacman position
        closestGhost = min(ghostPositions, key=lambda x: self.distancer.getDistance(pacmanPosition, x))

        def newPosition(action):
            return Actions.getSuccessor(pacmanPosition, action)

        return min(legal, key=lambda x: self.distancer.getDistance(newPosition(x), closestGhost))